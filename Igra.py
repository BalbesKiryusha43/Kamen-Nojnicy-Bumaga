...
class Application(tk.Frame):
    def __init__(self, master=None):
        ...
        # Варианты ходов (камень, бумага, ножницы)
        self.moves = ['rock', 'paper', 'scissor']

        # Очки пользователя и системы
        self.userPoint = 0
        self.sysPoint = 0
        ...
        # Словарь изображений для каждого хода
        self.imgDict = {'rock': rock_list, 'paper': paper_list, 'scissor': scissor_list}

    # Главный экран (ввод имени и кнопки)
    def main_frame(self):
    ...
    def play_game(self):
        if self.username.get():
            # Если имя введено, начинаем игру
            self.main.destroy()
            self.draw_frames()
            ...
            # Привязка событий (клики на иконки ходов)
            ...
            # Если имя не введено, показываем предупреждение
            messagebox.showinfo('Rock Paper Scissor', 'Player name is required')
            self.name_entry.focus_set()

    # Создание основных фреймов игры
    def draw_frames(self):
        ...
        # Отключаем пропорциональное изменение размеров
        self.header.grid_propagate(False)
        self.body.grid_propagate(False)
        self.footer.grid_propagate(False)

    # Рисование фрейма заголовка
    def draw_header_frame(self):
        ...
        # Фрейм для имени пользователя и его очков
        ...
        # Отображение имени пользователя и его очков
    ...
    # Рисование игрового фрейма
    def draw_body_frame(self):
        # Кнопки выбора (камень, бумага, ножницы) для пользователя
        ...
        # Кадры для показа выбранных ходов (пользователем и системой)
        ...
        # Метки для отображения результатов хода
    ...
    # Фрейм с кнопками внизу (сброс и возврат на главную)
    ...
    # Движение пользователя (выбор хода)
    def move(self, user_move):
        ...
        # Установка изображений для выбранных ходов
        if user_move == 'rock':
            image1 = rock_large
    ...
    # Проверка победителя на основе хода пользователя и системы
    ...
    # Обновление состояния игры после каждого раунда
    ...
    # Анимация при победе или поражении
    ...
    # Сброс игры (очистка результата и счетов)
    def reset_game(self):
        if self.after_id:
            self.after_cancel(self.after_id)
    ...
    # Возврат на главный экран (ввод имени)
    def go_home(self):
        self.reset_game()
        self.header.destroy()
        self.body.destroy()
